...
async function loadTaskId() {
  ...
  try {
    const response = await fetch(
      `http://localhost:8080/Project3-Backend/rest/task/${taskId}`,
      {
        method: "GET",
        headers: {
    ...
    if (task) {
      ...
      document.getElementById("date-start").value = task.startDate;

      document.getElementById("date-end").value = task.endDate;
    } else {
...
async function editTaskBE() {
  ...
  try {
    const response = await fetch(
      `http://localhost:8080/Project3-Backend/rest/task/edit/${taskId}`,
      {
